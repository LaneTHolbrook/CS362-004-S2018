...
int fullDeckCount(int player, int card, struct gameState *state) {
  int i;
  int count = state->deckCount[player] + state->handCount[player] + state->discardCount[player];
...
int adventurerEffect(int drawntreasure, int currentPlayer, struct gameState *state)
{
  ...
  // while(drawntreasure<2)
  while(drawntreasure<2) // FIXME bug introduced
  {
    ...
    drawCard(currentPlayer, state);
    cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
	if (cardDrawn < curse || cardDrawn > treasure_map)
	{
		break;
	}
    if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold)
...
int smithyEffect(int handPos, int currentPlayer, struct gameState *state)
{
  ...
  int i;
  // for (i = 0; i < 3; i++)
  for (i = 0; i < 3; i++) // FIXME introduced bug
  {
    drawCard(currentPlayer, state);
  }
